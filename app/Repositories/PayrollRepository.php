...
class PayrollRepository
{
    ...
    public function getEmployeeOtmAbsLvLtAndWokDaysManualWay($month, $employeeDetails)
    {
        ...
        if ($totalAbsence != 0 && $employeeDetails->basic_salary != 0 && $totalWorkingDays != 0) {
            $perDaySalary       = $employeeDetails->basic_salary / $totalWorkingDays;
            $totalAbsenceAmount = $perDaySalary * $totalAbsence;
        }
    ...
    public function makeEmployeeAdvanceDetuctionDataFormat($data)
    {
        ...
        $advanceDetuctionData['advancededuction_name'] = $data['advancededuction_name'];
    ...
    {
        $advanceDeductions = AdvanceDeduction::join('employee', 'employee.employee_id', '=', 'advance_deduction.employee_id')->where('advance_deduction.employee_id', $employee_id)->where('advance_deduction.payment_type', 1)->where('advance_deduction.status',0)->select('advance_deduction.*', 'employee.first_name', 'employee.last_name',)->get();
        $deductionArray    = [];
        $totalDeduction    = 0;
