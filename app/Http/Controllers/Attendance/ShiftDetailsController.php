...
class ShiftDetailsController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $validation = [
                ...
                '*.2' => 'required',
                '*.6' => function ($attribute, $value, $onFailure) use ($mon) {
                    $value = trim($value);
                    $arr = [$mon];
            ...
            $message = [
                ...
                '6.required' => 'Month field is required',
    ...
    public function export(Request $request)
    {
        ...
        $inc = 1;
        $workShiftDataset = [];
        $primaryHead = ['Sl.No', 'Employee ID', 'NAME', 'NATIONALITY', "RELIGION", 'GENDER', 'YYYY-MM'];
        ...
        foreach ($data as $key => $value) {
            ...
            if ($value['day_name'] == "Fri" || $value['day_name'] == "Sat") {
    ...
    {
        $cellRange = ["H", "I", "J", "K", "L", "M", "N", "O", "P", "Q", "R", "S", "T", "U", "V", "W", "X", "Y", "Z", "AA", "AB", "AC", "AD", "AE", "AF", "AG", "AH", "AI"];

        if ($countDays == 31) {
            $cellRange = array_merge($cellRange, ["AJ", "AK", "AL"]);
        } elseif ($countDays == 30) {
            $cellRange = array_merge($cellRange, ["AJ", "AK"]);
        } elseif ($countDays == 29) {
            $cellRange = array_merge($cellRange, ["AJ"]);
        }
