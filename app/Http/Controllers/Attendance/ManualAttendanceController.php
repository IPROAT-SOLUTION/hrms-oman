...
class ManualAttendanceController extends Controller
{
    ...
    public function ipAttendance(Request $request)
    {
        ...
        try {
            ...
            $count_user_login_today = EmployeeAttendance::where('finger_print_id', '=', $finger_id)->whereDate('in_out_time', '=', date('Y-m-d'))->count();
            if ($ip_check_status == 0) {
                $att = new EmployeeAttendance;
                $att->finger_print_id = $finger_id;
                $att->employee_id = decrypt(session('logged_session_data.employee_id'));
                ...
                $mssql = new MsSql;
                $mssql->ID = $finger_id;
                $mssql->datetime = date("Y-m-d H:i:s");
                $mssql->employee = decrypt(session('logged_session_data.employee_id'));
                $mssql->device_name = $user_ip;
                if ($count_user_login_today > 0 && $count_user_login_today % 2 != 0) {
                    $mssql->type = 'OUT';
                } else {
                    $mssql->type = 'IN';
                ...
                if ($check_white_listed > 0) {
                    ...
                    $mssql->employee = decrypt(session('logged_session_data.employee_id'));
                    $mssql->datetime = date("Y-m-d H:i:s");
                    $mssql->device_name = 'Ip Attendance';
                    if ($count_user_login_today > 0 && $count_user_login_today % 2 != 0) {
                        $mssql->type = 'OUT';
                    } else {
                        $mssql->type = 'IN';
