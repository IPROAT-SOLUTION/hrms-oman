...
class GenerateReportController extends Controller
{
    ...
    public function regenerateAttendanceReport(Request $request)
    {
        ...
        try {
            ...
            if ($request->department_id) {
                Employee::select('finger_id', 'employee_id','department_id')->status(UserStatus::$ACTIVE)->whereIn('department_id', $request->department_id)
                ...
                Employee::select('finger_id', 'employee_id','department_id')->status(UserStatus::$ACTIVE)->chunk(5, function ($employeeData) use ($datePeriod) {
                    foreach ($employeeData as $key => $employee) {
                        foreach ($datePeriod as $date) {
                            $date = $date->format('Y-m-d');
                            $in_time = '';
    ...
    public function shiftBasedReport($shift, $date, $month, $day, $finger_id)
    {
        ...
        if ($dailyShiftData->start_time > $dailyShiftData->end_time) {
            $nature = 'Night';
            $fdatetime = date('Y-m-d H:i:s', strtotime('-2 hours', strtotime($shiftStartTime)));
    ...
    public function overtimeLateEarlyCalc($data_format)
    {
        ...
        if ($data_format != []) {
            ...
            if (isset($data_format['work_shift_id']) && $data_format['work_shift_id'] != null) {
                ...
                $shift_list = WorkShift::orderBy('start_time', 'ASC')->oldest()->take(1)->get();
            ...
            if (isset($tempArray['work_shift_id']) && $tempArray['work_shift_id'] != null && isset($data_format['working_time']) && $data_format['working_time'] != null) {
                ...
                if ($workingTime > $naShiftDuration) {
                    ...
                    $tempArray['over_time'] = $this->find_overtime($over_time);
                } else {
                    $tempArray['over_time'] = null;
                }
    ...
    public function find_overtime($over_time)
    {
        $roundMinutes = (int) $over_time->i;
        $roundHours = (int) $over_time->h;
        $overtime = $roundHours . ':' . $roundMinutes;
        return $overtime;
    }
