...
class ApprovedOvertimeImport implements ToModel, WithValidation, WithStartRow, WithLimit
{
    ...
    public function model(array $row)
    {
        ...
        if ($isValid) {
            ...
            $employeeInOutData = EmployeeInOutData::where('finger_print_id', $approveOvertime->finger_print_id)->where('date', $approveOvertime->date)->update(['approved_over_time' => $row[4]]);
            ...
            $employeeInOutData = EmployeeInOutData::where('finger_print_id', $row[1])->where('date', $date)->update(['approved_over_time' => $row[4]]);
